  ...
  return (
    <form className="flex flex-col gap-6" onSubmit={handleSubmit}>
      ...
      <div className="flex flex-col items-start">
        ...
        />
        {isLoginFailed && (
          <p className="text-[#F87171] py-1.5 px-1 text-xs">{feedback}</p>
        )}
        <button
          type="button"
          className="text-[#374151] py-1.5 px-1 cursor-pointer text-xs"
          onClick={toggleClickForgotPassword}
        >
          Forgot password
        </button>
      </div>
      <button
        type="submit"
        className="btn bg-[#0C376C] text-[#C7D2FE] rounded-[8] h-12 text-xs font-light"
      >
        Sign in
      </button>
    </form>
